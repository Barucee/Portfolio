...
#fill the dataframe with the results of forecast_accuracy


def metrics_ARIMA(models_ARIMA, train_arima, test_arima):
    metrics_ARIMA = pd.DataFrame(columns=["Model", "mape", "me", "mae", "mpe", "rmse", "corr", "minmax"])
    for i in range(0,6):
        for j in range(0,6):
            metrics_ARIMA = metrics_ARIMA.append(forecast_accuracy(f"ARIMA{i}{d_optimum}{j}", models_ARIMA[f"ARIMA{i}{d_optimum}{j}"].predict(len(train_arima), len(train_arima)+len(test_arima)-1, typ='levels'), test_arima), ignore_index=True)
    return metrics_ARIMA

#apply the function to the models in multiple threads with multiprocessing
...
if pages == "Introduction 🗺️":
    ...
    if model == "ARIMA":
        ...
        st.write("""
                 According to Korstanje in his book, Advanced Forecasting with Python: “The LSTM cell 
